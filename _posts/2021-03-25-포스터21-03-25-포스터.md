...
2번의 경우는 최신영화 부분에 '입고순'을 하면 만개 이상의 최신 순 영화 정보를 얻을 수 있었습니다. 그래서 크롤링 대상 페이지를 **2번**으로 설정하고, 필요한 부분을 소스 보기의 HTML 태그 부분을 따와 크롤링을 진행했습니다. 여기서 19세 영화의 상세페이지를 누르면 로그인이 필요한데 이 때문에 단순히 반복 크롤링을 설정하면 도중에 멈추게 됩니다. 따라서 이를 방지하기 위해 예외 처리 작업을 해줘야합니다. 저는 연령 제한 영화는 대상에서 제외했기 때문에 해당 영화는 예외처리하여 크롤링을 진행했습니다. 이제 전체 크롤링 코드는 아래와 같습니다.
